...
def get_word_by_distence(file_name,word,is_force=False):
    ...
    if not os.path.exists(file_name) or is_force:
        with open(file_name, 'w', encoding='utf8') as f:
            context = bfs_get_word(wv, word, 5, 5)
            print(context)
            f.writelines('\n'.join(context))
        return  file_name
    else:
...
if __name__ == "__main__":
    ...
    get_word_find_path = os.path.abspath('../data/get_word2.txt')
    ...
    word2vector.generate_wordvector(news_data,is_train=False)
    ...
    word2vector.continue_train_wordvector(wiki_news_data,is_saver=True,is_train=False)

    print('get {} order by distence'.format('说'))

    word_file = get_word_by_distence(get_word_find_path,'说',True)

    #得到说的单词后，手动调整一下,从后往前删除
    word_file = os.path.abspath('../data/get_word_adjust.txt')
